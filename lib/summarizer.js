...
export async function generateSummary(config, title, content, options = {}) {
  ...
  if (mode === 'llm' || mode === 'auto') {
    const llmResult = await tryLLM(config, title, content, { length, requestId, pageType, signal });
  ...
  if (mode === 'tfidf' || mode === 'auto') {
    return tryTfidf(content, length);
  }
...
function tryTfidf(content, length) {
  ...
  try {
    const summary = tfidfSummarize(content, length);
    return {
      success: true,
      summary,
      method: 'tfidf'
