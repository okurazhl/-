...
function buildUserPrompt(title, content, length, pageType = 'article') {
  ...
  const visibleTextWithoutImages = String(content || '')
    .replace(/图片内容：[\s\S]*$/m, '')
    .replace(/https?:\/\/\S+/g, '')
    .trim();
  const imageGuide = /图片内容：/.test(content || '') && visibleTextWithoutImages.length < 220
...
- 使用简洁清晰的中文
- 保留关键数据和事实
- 遇到搜索结果页或列表页时，明确摘要基于页面条目而非原文全文${imageGuide}`;
}
