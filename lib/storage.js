...
function cloneDefaultSettings() {
  return {
    ...
    youtubeTranscriptApi: {
      ...DEFAULT_SETTINGS.youtubeTranscriptApi,
      preferredLanguages: [...DEFAULT_SETTINGS.youtubeTranscriptApi.preferredLanguages]
    }
  };
}
...
function mergeSettingsWithDefaults(settings = {}) {
  ...
  const savedTranscriptApi = safeSettings.youtubeTranscriptApi || {};
  const shouldApplyLocalTranscriptApi =
    !savedTranscriptApi.endpoint ||
    !savedTranscriptApi.apiKey;
  const youtubeTranscriptApi = {
    ...DEFAULT_SETTINGS.youtubeTranscriptApi,
    ...savedTranscriptApi,
    ...(shouldApplyLocalTranscriptApi ? LOCAL_YOUTUBE_TRANSCRIPT_API : {}),
    preferredLanguages: Array.isArray(savedTranscriptApi.preferredLanguages) ?
      savedTranscriptApi.preferredLanguages :
      [...DEFAULT_SETTINGS.youtubeTranscriptApi.preferredLanguages]
  };
...
export async function initStorage() {
  ...
  initPromise = (async () => {
    try {
      ...
      const needLocalTranscriptApiDefaults = !result.settings?.youtubeTranscriptApi?.endpoint ||
        !result.settings?.youtubeTranscriptApi?.apiKey;
      ...
      if (needDefaultCategories || needDefaultSettings || needLocalTranscriptApiDefaults || needDefaultMetadata) {
...
export async function createNote(noteData = {}) {
  ...
  const note = {
    ...
    imageOcr: noteData.imageOcr || null,
    noiseSelectionMethod: noteData.noiseSelectionMethod || '',
    noiseSelectionConfidence: typeof noteData.noiseSelectionConfidence === 'number' ? noteData.noiseSelectionConfidence : null,
    noiseSelectionReason: noteData.noiseSelectionReason || '',
    categoryId: noteData.categoryId || settingsCache.defaultCategoryId || '',
    tags: noteData.tags || [],
    summary: noteData.summary || '',
    summaryMethod: noteData.summaryMethod || '',
    summaryStatus: noteData.summaryStatus || '',
    summarySavedAt: noteData.summarySavedAt || null,
    summaryUsage: noteData.summaryUsage || null,
    createdAt: now,
    updatedAt: now,
...
export function getSettings() {
  return {
    ...settingsCache,
    privacy: { ...settingsCache.privacy },
    llm: { ...settingsCache.llm },
    youtubeTranscriptApi: {
      ...settingsCache.youtubeTranscriptApi,
      preferredLanguages: Array.isArray(settingsCache.youtubeTranscriptApi?.preferredLanguages) ?
        [...settingsCache.youtubeTranscriptApi.preferredLanguages] :
        [...DEFAULT_SETTINGS.youtubeTranscriptApi.preferredLanguages]
    }
  };
}
...
export async function updateSettings(updates = {}) {
  ...
  if (nextUpdates.youtubeTranscriptApi) {
    settingsCache.youtubeTranscriptApi = {
      ...(settingsCache.youtubeTranscriptApi || {}),
      ...nextUpdates.youtubeTranscriptApi
    };
    delete nextUpdates.youtubeTranscriptApi;
  }
