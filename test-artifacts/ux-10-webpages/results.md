...
| 3 | [Wikipedia Readability](https://en.wikipedia.org/wiki/Readability) | 百科长文 | usable | 通过 | article | 50191 | readability | Readability: usable |
| 4 | [Example.com](https://example.com/) | 短网页 | usable-with-warning | 通过 | unknown | 115 | readability | Example Domain: usable |
| 5 | [Hacker News 首页](https://news.ycombinator.com/) | 列表/聚合页 | usable-with-warning | 通过 | listing | 1468 | listing | Hacker News: usable |
...
| 8 | [YouTube 视频页](https://www.youtube.com/watch?v=dQw4w9WgXcQ) | 视频页 | partial | 通过 | video | 2244 | youtube-metadata | Rick Astley: possible-boilerplate-or-gate-text |
...
| 10 | [阮一峰周刊文章](https://www.ruanyifeng.com/blog/2024/06/weekly-issue-306.html) | 受限/反爬页面 | usable | 通过 | article | 4699 | readability | 科技爱好者周刊（第 306 期）：信息就像糖一样上瘾: usable |
...
- 字数: 240
- 质量提示: 页面类型不明确，已使用通用正文提取。；当前页面未匹配专用提取器，已使用通用回退提取。；检测到正文图片，但当前浏览器未提供可用 OCR 能力，已保留图片信息。
...
- 预览: 页面类型：列表/聚合页 页面标题：Hacker News 以下为页面中的主要列表条目： 1. Deno Desktop 说明：316 points · 121 comments 2. GLM 5.2 vs. Opus 说明：105 points · 64 comments 3. Codex logging bug may write TBs to local SSDs 说明：59 points · 30 comments 4. Help I accidentally a wigglegram 说明：218 points · 38 comments 5. Did my old job only exist because of fra
...
- 状态: partial; sidepanel 显示 通过
- 标题: Rick Astley
- 类型/方法: video / youtube-metadata
- 字数: 2244
- 质量提示: 未能获取 YouTube 字幕/Transcript；摘要只能基于标题、简介和章节信息，不能代表完整视频内容。；YouTube 字幕读取失败：页面未显示可读取的 Transcript DOM；captionTracks 返回为空；页面未暴露可用 captionTracks；get_transcript 请求失败 HTTP 400；页面未找到可点击的 Transcript 入口；当前页面可能是视频/音频页；如果没有字幕或简介，正文提取可能不完整。
- 预览: 未能获取 YouTube 字幕/Transcript；摘要只能基于标题、简介和章节信息，不能代表完整视频内容。 YouTube 视频标题：Rick Astley 频道：Rick Astley 简介： The official video for “Never Gonna Give You Up” by Rick Astley. Never: The Autobiography 📚 OUT NOW! Follow this link to get your copy and listen to Rick’s ‘Never’ playlist ❤️ #RickAstleyNever “Never Gonna Give You Up” 
...
- 状态: usable; sidepanel 显示 通过
- 标题: 科技爱好者周刊（第 306 期）：信息就像糖一样上瘾
- 类型/方法: article / readability
- 字数: 4699
- 质量提示: 无
- 预览: 科技爱好者周刊（第 306 期）：信息就像糖一样上瘾作者： 阮一峰这里记录每周值得分享的科技内容，周五发布。 本杂志开源，欢迎投稿。 另有《谁在招人》服务，发布程序员招聘信息。 合作请邮件联系（yifeng.ruan@gmail.com）。 封面图香港屯门码头，海边的夜色。 （via）信息就像糖一样上瘾工业革命有一个很少人提到的后果，就是人类可以大量生产糖了。 古时候，糖主要来自甘蔗和蜂蜜，难以大量生产。 所以，古人的菜肴不放糖，主要依靠食物本身的甜味。 可以想象，古代的大部分食物，尤其是日常食品，应该是不好吃的。 一旦糖大量生产，人类就疯狂地爱上了它。 我们喜欢糖，喜欢到现在的大部分食物，都必须添加糖，比如饮料、糕点、奶制品，甚至
